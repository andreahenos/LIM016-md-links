...
export const verifyFileType = (route) => {
  fs.lstat(route, (err, stats) => {
    ...
    if (stats.isDirectory()) return console.log('2. Stats: Is a directory');
  });
};
...
export const readDirectory = (route) => {
  fs.readdir(route, (err, files) => {
    if (err) return console.log('3. Ha ocurrido un error.');
...
export const readFiles = (route) => {
  fs.readFile(route, 'utf8', (err, content) => {
    if (err) return console.log('4. Ha ocurrido un error');
